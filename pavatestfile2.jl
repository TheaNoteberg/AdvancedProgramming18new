...
getproperty(c1, :real) # should be 1
c1.real # 1
...
# Test the new method
my_class = MyClass(:foo, :bar, Dict(:foo => 42, :bar => "hello"))
my_instance = new(my_class, foo=13, baz="world")
println(my_instance.slots)  # should print Dict(:foo=>13, :bar=>"hello", :instance_of=>MyClass(:foo, :bar), :baz=>"world")
...
@defmethod add(a::Complex, b::Complex) = a + b

# Test the generic function with different inputs
@test add(1, 2) == 3
@test add(3.5, 2.1) == 5.6
@test add("hello", "world") == "helloworld"
@test add(1+2im, 3+4im) == 4+6im

# Test the generic function with invalid inputs
@test_throws MethodError add(true, false)
@test_throws MethodError add([1, 2, 3], [4, 5, 6])
...
class_of(c1) # should be ComplexNumber
ComplexNumber.direct_slots # should be :real, :imag
class_of(class_of(c1)) # should be Class
class_of(class_of(class_of(c1))) # should be class
Class.slots # should be :name, :direct_superclasses, :direct_slots, ...
ComplexNumber.name # should be :ComplexNumber
ComplexNumber.direct_superclasses == [Object] # should be true
...
foobar1 = new(FooBar) # <FooBar 6i1nVhjh6o>
foobar1.a #1
foobar1.b #3
foobar1.c #4
foobar1.d #6
